...
const focusNextInput = (currentValue: string, nextInput: HTMLInputElement | null, maxLength: number) => {
...
const updateRegistrationDate = () => {
  if (registrationDay.value && registrationMonth.value && registrationYear.value) {
    vehicle.value.dateOfFirstRegistration = `${registrationYear.value}-${registrationMonth.value.padStart(2, '0')}-${registrationDay.value.padStart(2, '0')}`;
  }
};
...
const updatePurchaseDate = () => {
  if (purchaseDay.value && purchaseMonth.value && purchaseYear.value) {
    vehicle.value.dateOfPurchase = `${purchaseYear.value}-${purchaseMonth.value.padStart(2, '0')}-${purchaseDay.value.padStart(2, '0')}`;
  }
};
</script>
...
<template>
    ...
    <div class="mb-6">
      <label class="block text-gray-700 mb-2">Date de première immatriculation du véhicule</label>
      <div class="flex space-x-4">
        <!-- Input pour le jour -->
        <input
          v-model="registrationDay"
          @input="focusNextInput(registrationDay, registrationMonthInputRef, 2); updateRegistrationDate"
          type="text"
          maxlength="2"
          placeholder="Jour"
        ...
        <input
          ...
          @input="focusNextInput(registrationMonth, registrationYearInputRef, 2); updateRegistrationDate"
          type="text"
          maxlength="2"
          placeholder="Mois"
          class="w-full p-2 border border-gray-300 rounded-lg"
    ...
    <div class="mb-6">
      ...
      <div class="flex space-x-4">
        ...
        <input
          v-model="purchaseDay"
          @input="focusNextInput(purchaseDay, purchaseMonthInputRef, 2); updatePurchaseDate"
          type="text"
          maxlength="2"
          placeholder="Jour"
          class="w-full p-2 border border-gray-300 rounded-lg"
        ...
        <input
          ...
          @input="focusNextInput(purchaseMonth, purchaseYearInputRef, 2); updatePurchaseDate"
          type="text"
          maxlength="2"
          placeholder="Mois"
          class="w-full p-2 border border-gray-300 rounded-lg"
